    ...
    <div className="w-full h-[65px] fixed top-0 shadow-lg shadow-[#2A0E61]/50 bg-[#03001427] backdrop-blur-md z-50 px-10">
      ...
      <div className="w-full h-full flex items-center justify-between m-auto px-[10px]">
        ...
        <div className="hidden md:flex w-auto h-full flex-row items-center justify-between md:mr-20">
          <div className="flex items-center justify-between gap-6 w-full h-auto border-[rgba(112,66,248,0.38)] bg-[rgba(3,0,20,0.37)] mr-[15px] px-[25px] py-[10px] rounded-full text-gray-200">
            {NAV_LINKS.map((link) => (
              <Link
                key={link.title}
                href={link.link}
                className="cursor-pointer hover:text-[rgb(112,66,248)] transition text-[15px]"
              >
                {link.title}
              </Link>
            ))}
